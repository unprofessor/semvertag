...
`v1.0.0` sorts **before** `v1.0.0` &mdash; the opposite of what you meant. This crate
...
1. Tag at HEAD, clean &rarr; the tag as-is (`v1.0.0` &rarr; `1.0.0`).
2. Tag at HEAD, dirty &rarr; tag with `+dirty` build metadata (provenance, not ordering).
3. Commits past a plain release &rarr; patch bump + `dev.{N}` prerelease + `g{hash}` build metadata.
4. Commits past a prerelease tag &rarr; keep the version, append `.dev.{N}` to the prerelease + `g{hash}` build metadata.

A leading `v`/`V` is stripped from tags. 0.x versions are treated like any other
major &mdash; no Cargo-style left-shifted semantics.

## When to use this crate directly

`semvertag-core` is the right choice when you already invoke `git` yourself (or
use `git2`) and only need the derivation &mdash; for example in a `build.rs`, where
